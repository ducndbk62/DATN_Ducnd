...
public class EnemyController : MonoBehaviour
{
    ...
    public float accuracy; // time update rotate gun
    private float lastRotateGun;
    ...
    void UpdateFireTime()
    {
        ...
        if (minFireTime > 2)
        {
            if ((System.Math.Truncate(Time.time) + 1) % 10 == 0)
            {
                minFireTime /= 2;
                maxFireTime /= 2;
            }
    ...
    void Fire()
    {
        if (Time.time >= lastRotateGun + accuracy)
        {
            RotateGun();
            lastRotateGun = Time.time;
        }
